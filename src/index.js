...
import { startBolnaSyncJob } from './cron/bolnaSync.cron.js';

let server;
let orderSyncCronJob;
let bolnaSyncCronJob;
...
mongoose.connect(config.mongoose.url, config.mongoose.options).then(async () => {
  ...
  // Start Bolna call status sync cron job
  // Syncs every 1 minute by default to keep call statuses updated
  const bolnaSyncSchedule = process.env.BOLNA_SYNC_CRON_SCHEDULE || '* * * * *'; // Every 1 minute
  bolnaSyncCronJob = startBolnaSyncJob(bolnaSyncSchedule);
